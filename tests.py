...
class US04Test(unittest.TestCase):
    ...
    def test_no_div(self):
        entry = {"id": "f01", "marr": date(1992,5,23)}
        with captured_output() as (out,err):
    ...
    def test_marr_before_div(self):
        entry = {"id": "f01", "marr": date(1992,5,23), "div": date(2017,3,4)}
        with captured_output() as (out,err):
            run.add_entry(entry, "FAM")
        output = out.getvalue().strip()
        self.assertEquals("", output)
    ...
    def test_div_before_marr(self):
        entry = {"id": "f02", "marr": date(1998,3,25), "div": date(1993,4,6)}
        with captured_output() as (out,err):
            run.add_entry(entry, "FAM")
        output = out.getvalue().strip()
    ...
    def test_marr_div_same(self):
        entry = {"id": "f03", "marr": date.today(), "div": date.today()}
        with captured_output() as (out,err):
            run.add_entry(entry, "FAM")
        output = out.getvalue().strip()
        self.assertEquals("", output)
